# Publishing Voidwave v1.7.0 with working Pelican updates
...
Confirm both raw manifests show version 1.7.0:
...
```bash
curl -fL \
  https://github.com/GalaxyFSRP1/voidwave-pelican-theme/releases/download/v1.7.0/voidwave-theme.zip \
  -o /tmp/voidwave-theme.zip
unzip -t /tmp/voidwave-theme.zip
unzip -p /tmp/voidwave-theme.zip plugin.json | grep version
```

The URL must return HTTP 200, the ZIP test must pass, and the version must be 1.7.0.
